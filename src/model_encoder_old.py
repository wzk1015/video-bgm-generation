...
class BaseModel(nn.Module):
    ...
    def inference_from_scratch(self, vlog=None, o_den_track_list=(1,2,3), pre_init=None, visualize=False, C=1.0):
        ...
        with torch.no_grad():
            ...
            pre_init_t = torch.from_numpy(pre_init).long().cuda()
            print('------ pre_initiate ------')
            for step in range(pre_init.shape[0]):
                input_ = pre_init_t[step, :].unsqueeze(0).unsqueeze(0)
                print(input_)
                h, y_type, memory = self.forward_init_token(
            ...
            if condition:
                ...
                while (True):
                    ...
                    # end of sequence
                    # if word2event['type'][next_arr[1]] == 'EOS':
                    #     print("EOF predicted")
                    #     break
                    #
                    # if word2event['bar-beat'][next_arr[0]] == 'Bar':
                    #     cnt_bar += 1
